...
class UserCacheUpdatetStreamTaskSpecV2 extends BaseTestSpec with BeforeAndAfterEach {
  ...
  def setupRestUtilData(): Unit = {
    ...
    server.url("http://127.0.0.1:3000/userorg/private/user/v1/read/user-1")
    server.enqueue(new MockResponse().setBody(user1))
//    server.enqueue(new MockResponse().setBody(clientError))
    server.url("http://127.0.0.1:3000/userorg/private/user/v1/read/user-9")
    server.enqueue(new MockResponse().setBody(user1))
//    server.enqueue(new MockResponse().setBody(user2))
    server.url("http://127.0.0.1:3000/userorg/private/user/v1/read/user-2")
    server.enqueue(new MockResponse().setBody(user3))
    server.url("http://127.0.0.1:3000/userorg/private/user/v1/read/user-3")
    server.enqueue(new MockResponse().setBody(user4))
    server.url("http://127.0.0.1:3000/userorg/private/user/v1/read/user-4")
    server.enqueue(new MockResponse().setBody(user1))
//    server.enqueue(new MockResponse().setBody(user2))
    server.url("http://127.0.0.1:3000/userorg/private/user/v1/read/user-5")
    server.enqueue(new MockResponse().setBody(user10))
    server.url("http://127.0.0.1:3000/userorg/private/user/v1/read/user-10")
...
//    server.url("http://127.0.0.1:3000/api/dialcode/v3/read/X3J6W3")
  ...
  "UserCacheUpdater" should "be able to add user to cache with all information" in {
    ...
    userInfo.get("framework_board") should be ("""["TestBoard-Health"]""")
    userInfo.get("rootorgid") should be ("0127738024883077121")
    userInfo.get("orgname") should be ("Root Org2")
    userInfo.get("framework_subject") should be ("""["IRCS"]""")
    userInfo.get("language") should be ("""["English"]""")
    userInfo.get("framework_gradeLevel") should be ("""["Volunteers"]""")
    userInfo.get("framework_id") should be ("testboard_health")
    userInfo.get("framework_medium") should be ("""["English"]""")
    ...
    userInfo.get("orgname") should be ("Root Org2")
    userInfo.get("framework_board") should be (null)
    userInfo.get("framework_subject") should be (null)
    userInfo.get("language") should be ("""[]""")
    userInfo.get("framework_gradeLevel") should be (null)
    userInfo.get("framework_id") should be (null)
    userInfo.get("framework_medium") should be (null)
