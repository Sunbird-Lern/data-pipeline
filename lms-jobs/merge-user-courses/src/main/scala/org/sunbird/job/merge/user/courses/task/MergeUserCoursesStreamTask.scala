...
class MergeUserCoursesStreamTask(config: MergeUserCoursesConfig, kafkaConnector: FlinkKafkaConnector) {
  def process(): Unit = {
    ...
    val progressStream = env.fromSource(source, WatermarkStrategy.noWatermarks[Event](), config.mergeUserCoursesConsumer)
    ...
    progressStream.getSideOutput(config.eventOutputTag).sinkTo(kafkaConnector.kafkaStringSink(config.courseBatchUpdaterTopic))
