...
class UserDeletionCleanupFunctionTestSpec extends BaseTestSpec {
  ...
  "UserDeletionCleanupStreamTaskProcessor " should "validate metrics " in {
    ...
    val inputStream = env.addSource(new UserDeletionCleanupEventSource).name(jobConfig.userDeletionCleanupConsumer)
      .uid(jobConfig.userDeletionCleanupConsumer).setParallelism(jobConfig.userDeletionCleanupParallelism).rebalance
    new UserDeletionCleanupStreamTask(jobConfig, mockHttpUtil, mockKafkaUtil).processForTest(env, inputStream)
