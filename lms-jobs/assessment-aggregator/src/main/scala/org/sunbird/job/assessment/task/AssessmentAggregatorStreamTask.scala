...
import org.sunbird.job.assessment.functions.{AssessmentAggregatorFunction, UserScoreAggregateFunction}
...
class AssessmentAggregatorStreamTask(config: AssessmentAggregatorConfig, kafkaConnector: FlinkKafkaConnector) {
    ...
    def process(): Unit = {
        ...
        val aggregatorStream =
            env.addSource(source).name(config.assessmentAggConsumer)
              .uid(config.assessmentAggConsumer).setParallelism(config.kafkaConsumerParallelism).rebalance
              .process(new AssessmentAggregatorFunction
              (config))
