...
class CertificateGeneratorFunctionTaskTestSpec extends BaseTestSpec {
  ...
  it should "generate certificate and add to the registry" in {
    ...
    val inputStream = env.addSource(new CertificateGeneratorPreProcessorEventSource)
      .name(jobConfig.notificationConsumer)

    intercept[JobExecutionException](new CertificateGeneratorStreamTask(jobConfig, mockKafkaUtil, mockHttpUtil).processForTest(env, inputStream))
...
class CertificateGeneratorPreProcessorEventSource extends SourceFunction[org.sunbird.job.collectioncert.domain.Event] {
  override def run(ctx: SourceContext[org.sunbird.job.collectioncert.domain.Event]): Unit = {
    ...
    ctx.collect(new org.sunbird.job.collectioncert.domain.Event(eventMap1.asInstanceOf[util.Map[String, Any]], 0, 0))
    ctx.collect(new org.sunbird.job.collectioncert.domain.Event(eventMap3.asInstanceOf[util.Map[String, Any]], 0, 1))
    ctx.collect(new org.sunbird.job.collectioncert.domain.Event(eventMap2.asInstanceOf[util.Map[String, Any]], 0, 2))
  }
