...
class CertMapper(certConfig: CertificateConfig) {
  ...
  def mapReqToCertModel(certReq: Event): CertModel = {
    val data: Map[String, AnyRef] = certReq.data.head
    ...
    CertModel(recipientName = data.getOrElse(JsonKeys.RECIPIENT_NAME, "").asInstanceOf[String],
  ...
  private def getIssuer(req: Event): Issuer = {
    ...
    val publicKeys: List[String] = getPublicKeys(issuerData.getOrElse(JsonKeys.PUBLIC_KEY, List[String]()).asInstanceOf[List[String]], req.keys)
